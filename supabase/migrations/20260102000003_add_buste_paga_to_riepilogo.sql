-- Migration: Add buste paga costs to riepilogo_economico_commessa
-- Description: Aggiunge i costi delle buste paga suddivisi per commessa al riepilogo economico
-- Date: 2026-01-02

-- Drop the existing view
DROP VIEW IF EXISTS public.riepilogo_economico_commessa;

-- Recreate with buste_paga costs
...
SELECT
  ...
  -- Totali (inclusi costi buste paga)
  COALESCE(cf.costi_imponibile, 0) + COALESCE(cbp.costi_buste_paga, 0) as costi_imponibile_totali,
  COALESCE(cf.costi_totali, 0) + COALESCE(cbp.costi_buste_paga, 0) as costi_totali_completi,
  COALESCE(r.numero_ricavi, 0) + COALESCE(cf.numero_fatture_passive, 0) as numero_movimenti_totali,
  -- Margini (con buste paga)
  COALESCE(r.ricavi_totali, 0) - (COALESCE(cf.costi_totali, 0) + COALESCE(cbp.costi_buste_paga, 0)) as margine_lordo,
  CASE
    WHEN COALESCE(r.ricavi_totali, 0) > 0 THEN
      ((COALESCE(r.ricavi_totali, 0) - (COALESCE(cf.costi_totali, 0) + COALESCE(cbp.costi_buste_paga, 0))) / COALESCE(r.ricavi_totali, 0)) * 100
    ELSE 0
...
-- Add comment
COMMENT ON VIEW public.riepilogo_economico_commessa IS 'View che aggrega i dati economici per commessa (ricavi da fatture attive, costi da fatture passive e buste paga)';
