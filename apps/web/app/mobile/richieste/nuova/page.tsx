...
import { ArrowLeft, Upload, Calendar, Euro, FileText, Trash, Tag, Check } from 'lucide-react';
...
type TipoRichiesta = 'nota_spesa' | 'ferie' | 'permesso';
...
export default function NuovaRichiestaPage() {
  ...
  const { dipendente } = useMobileData();

  // Tipo richiesta
  const [tipoRichiesta, setTipoRichiesta] = useState<TipoRichiesta | ''>('');
    ...
    <div className="space-y-6 pb-24">
      ...
      <div className="bg-emerald-600 px-6 py-8 text-white">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-xl font-bold">Nuova Richiesta</h1>
        </div>
      </div>
        ...
        <div className="bg-white rounded-3xl shadow-xl p-5 border border-gray-100">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              <Label htmlFor="tipo_richiesta" className="text-sm font-semibold text-gray-900">
                Tipo Richiesta <span className="text-red-600">*</span>
              </Label>
              <Select value={tipoRichiesta} onValueChange={(value) => setTipoRichiesta(value as TipoRichiesta)}>
                ...
                <SelectContent>
                  <SelectItem value="nota_spesa">Nota Spesa</SelectItem>
                  <SelectItem value="ferie" disabled>Ferie (Prossimamente)</SelectItem>
                  <SelectItem value="permesso" disabled>Permesso (Prossimamente)</SelectItem>
                </SelectContent>
              </Select>
            </div>

            {/* Form Nota Spesa - Solo se tipo è nota_spesa */}
            {tipoRichiesta === 'nota_spesa' && (
              <>
                {/* Commessa */}
                <div className="space-y-2">
            ...
            {/* Messaggio se non è selezionato tipo richiesta */}
            {!tipoRichiesta && (
              <div className="text-center py-12">
                <p className="text-gray-500 text-sm">
                  Seleziona un tipo di richiesta per continuare
                </p>
              </div>
            )}

            {/* Messaggi per tipi non ancora implementati */}
            {tipoRichiesta && tipoRichiesta !== 'nota_spesa' && (
              <div className="text-center py-12">
                ...
                <p className="text-sm text-gray-500">
                  La gestione di {tipoRichiesta === 'ferie' ? 'ferie' : 'permessi'} sarà disponibile a breve
                </p>
              </div>
            )}
          </form>
