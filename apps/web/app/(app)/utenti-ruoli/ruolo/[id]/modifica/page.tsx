...
import { getRoleById, updateCustomRole } from '@/lib/roles';
import type { RolePermissions, CustomRole } from '@/lib/roles';
import {
  Loader2,
  Briefcase,
  Users,
...
export default function ModificaRuoloPage() {
  ...
  const loadRole = async () => {
    try {
      ...
      setSelectedIconName(roleData.icon || 'Lock');
    } catch (error) {
  ...
  const handlePermissionToggle = (section: keyof RolePermissions, action: string) => {
    // Non permettere modifiche ai ruoli di sistema
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const success = await updateCustomRole(roleId, {
        name: name.trim(),
      ...
      {isSystemRole && (
        <div className="rounded-lg border-2 border-blue-200 bg-blue-50 p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-blue-900 mb-1">Ruolo di Sistema - Sola Lettura</h3>
              <p className="text-sm text-blue-700">
                Questo è un ruolo predefinito del sistema. I permessi sono visualizzabili ma non modificabili per garantire la sicurezza e la coerenza del sistema.
              </p>
            </div>
          </div>
        </div>
                ...
                <Popover open={showIconPicker} onOpenChange={setShowIconPicker}>
                  <PopoverTrigger asChild>
                    <button
                      ...
                      className="p-3 rounded-lg border-2 border-border bg-background hover:border-emerald-500 hover:bg-emerald-50/50 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {(() => {
