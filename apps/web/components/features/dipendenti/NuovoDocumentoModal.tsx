...
import type { TipoDocumento, TIPI_DOCUMENTO } from '@/types/documento-dipendente';

interface NuovoDocumentoModalProps {
  onClose: () => void;
  onSuccess: () => void;
  dipendenteId: string;
}

const TIPI_DOCUMENTO_OPTIONS: Record<TipoDocumento, string> = {
...
export function NuovoDocumentoModal({ onClose, onSuccess, dipendenteId }: NuovoDocumentoModalProps) {
  ...
  const handleFiles = (files: File[]) => {
    ...
    // Create preview URL (for PDFs/images)
    ...
    try {
      ...
      const { error: insertError } = await supabase
        ...
        .insert({
          ...
          ente_rilascio: enteRilascio.trim() || null,
          created_by: user.id,
        });
    ...
    <ModalWrapper onClose={onClose}>
      <div className="max-w-3xl w-full mx-auto max-h-[90vh] overflow-y-auto rounded-xl border-2 border-border bg-card shadow-lg animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between p-6 border-b-2 border-border sticky top-0 bg-card z-10">
          <div>
            <h2 className="text-xl sm:text-2xl font-bold">Nuovo Documento</h2>
            <p className="text-sm text-muted-foreground mt-1">
              Carica un documento del dipendente
            </p>
          </div>
          <Button
            variant="ghost"
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="space-y-2">
            <Label htmlFor="tipo">Tipo Documento <span className="text-destructive">*</span></Label>
            <Select value={tipoDocumento} onValueChange={(value) => setTipoDocumento(value as TipoDocumento)}>
              <SelectTrigger className="bg-white border border-input h-11">
                <SelectValue placeholder="Seleziona tipo documento" />
              </SelectTrigger>
              <SelectContent>
                {Object.entries(TIPI_DOCUMENTO_OPTIONS).map(([key, label]) => (
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="es. Patente di guida categoria B"
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Data Rilascio */}
            <div className="space-y-2">
              <Label htmlFor="data-rilascio">Data Rilascio</Label>
              <Input
                id="data-rilascio"
                type="date"
                value={dataRilascio}
            ...
            {/* Numero Documento */}
            <div className="space-y-2">
              <Label htmlFor="numero">Numero Documento</Label>
              <Input
                id="numero"
            ...
            {/* Ente Rilascio */}
            <div className="space-y-2">
              <Label htmlFor="ente">Ente di Rilascio</Label>
              <Input
                id="ente"
                value={enteRilascio}
                onChange={(e) => setEnteRilascio(e.target.value)}
                className="bg-white border border-input"
                placeholder="es. MIT - Motorizzazione"
              />
            </div>
          </div>
          ...
          {/* File Upload */}
          <div className="space-y-2">
            <Label>Allegato <span className="text-destructive">*</span></Label>
            <div
              className={cn(
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-border">
            ...
            <Button
              type="submit"
              disabled={loading || !selectedFile || !tipoDocumento || !nomeDocumento.trim()}
